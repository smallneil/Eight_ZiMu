package com.geetol.videoedit.whole.selCover;
...
import com.geetol.videoedit.whole.selCover.view.ThumbnailSelTimeView;
...
public class SelCoverTimeActivity extends AppCompatActivity {
    ...
    ImageView mIvBack;
    @BindView(R2.id.cut_time_finish_tv)
    TextView mCutTimeFinishTv;
    @BindView(R2.id.rl_title)
    RelativeLayout mRlTitle;
    @BindView(R2.id.cut_recycler_view)
    RecyclerView mCutRecyclerView;
    @BindView(R2.id.thumb_sel_time_view)
    ThumbnailSelTimeView mThumbSelTimeView;
    @BindView(R2.id.sel_cover_video_view)
    VideoView mSelCoverVideoView;
    @BindView(R2.id.sel_cover_tv)
    TextView mSelCoverTv;
    @BindView(R2.id.sel_cover_frame_layout)
    ...
    @OnClick({R2.id.iv_back, R2.id.cut_time_finish_tv})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R2.id.iv_back:
                onBackPressed();
                break;
            case R2.id.cut_time_finish_tv:
                mIsSelTime = true;
                onBackPressed();
                break;
        }
