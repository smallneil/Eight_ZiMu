package com.geetol.videoedit.blocks.mediaCodec.recordBaseCamera;
...
public class RecordBaseCameraActivity extends BaseActivity implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    @BindView(R2.id.record_base_camera_sv)
    SurfaceView mRecordBaseCameraSv;
    @BindView(R2.id.btn_record_base_status)
    Button mBtnRecordBaseStatus;
    @BindView(R2.id.btn_record_base_player)
...
//                    mParameters.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH);//预览持续发光
    ...
    @OnClick({R2.id.btn_record_base_status, R2.id.btn_record_base_player})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R2.id.btn_record_base_status:
                ...
                break;
            case R2.id.btn_record_base_player:
