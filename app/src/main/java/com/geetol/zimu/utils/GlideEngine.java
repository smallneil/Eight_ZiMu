...
public class GlideEngine implements ImageEngine {
     ...
     * @param corner 倒角 dp
     */
    public void loadPhoto(Context context, String photoPath, ImageView imageView,int corner) {
        Glide.with(context)
                .load(photoPath)
                .transition(withCrossFade())
                .apply(RequestOptions.bitmapTransform(new RoundedCorners(DisplayUtil.dip2px(corner))))
                .into(imageView);
        //圆角
        //apply(RequestOptions.bitmapTransform(new RoundedCorners(20)))
    }
